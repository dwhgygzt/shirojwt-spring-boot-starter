...
public class JwtRolesAuthorizationFilter extends RolesAuthorizationFilter {
    ...
    public static void setJwtBussinessService(JwtBussinessService jwtBussinessService) {
        JwtRolesAuthorizationFilter.jwtBussinessService = jwtBussinessService;
    }
    ...
    public boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) throws IOException {
        ...
        if (!allowed) {
            ShiroExceptionContext.setCurrentException(new UnauthorizedException("Subject does not have role" + Arrays.toString(((String[]) mappedValue))));
        }
    ...
    protected boolean onAccessDenied(ServletRequest servletRequest, ServletResponse servletResponse) {
        if (logger.isDebugEnabled()) {
            logger.debug("角色检查失败, 执行onAccessDenied 方法...");
        }
        try {
